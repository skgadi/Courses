...
\begin{document}
	...
	{%
		...
		\section*{Questions}
		\begin{enumerate}
			\item  In the following RLC series circuit, $R = \SI{{\VarOOA}0}{\ohm}$, $L = \SI{{\VarOOB}0}{\milli\henry}$, $C = \SI{{\VarOOC}}{\micro\farad}$ and $V = \mathop{\mathrm{heaviside}}\nolimits\!\left(t\right)$ (Unit step). Plot voltage across the each element and the circuit current. (10 point)\\
			\begin{minipage}{\textwidth}
				\centering
				\includegraphics[height=4cm]{images/Drawing1}\\
				\textbf{Figure 1}
			\end{minipage}
			\item Find resonant frequency for the above circuit. (1 point)
		\end{enumerate}
